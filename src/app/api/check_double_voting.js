...
async function checkDoubleVoting(){
    let allowedToVote = [];
    let delegatedVote = [];
    for (const member in members){
        const childs = await getAttestationsForAddress(members[member]);
        // console.log("Member:", members[member]);
        if (childs.length === 0){
            allowedToVote.push(members[member]);
        }else{
            delegatedVote.push(members[member]);
        }   
    }
    return [ allowedToVote, delegatedVote ];
}

let allowedToVote = "", delegatedVote = "";
[allowedToVote, delegatedVote] = await checkDoubleVoting();
console.log("Allowed to vote:", allowedToVote);
console.log("Not allowed to vote (have delegated):", delegatedVote);

async function checkFurtherDelegation(delegatedVoters){
    for (const dele in delegatedVoters){
        const attestations = await getAttestationsForAddress(delegatedVoters[dele]);
        for (const a in attestations){
            const currentBoolState = JSON.parse(attestations[a].decodedDataJson)[0].value.value;
            // console.log(currentBoolState);
            if (!currentBoolState){
                // TODO here you have to rearrange the graph.
            }
        }
    }
}

await checkFurtherDelegation(delegatedVote);
